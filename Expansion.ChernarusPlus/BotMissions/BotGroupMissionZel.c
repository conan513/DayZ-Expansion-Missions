class BotGroupMissionZel: CustomMission
{
	
	void BotGroupMissionZel() {}
	
	
	// Конфиги ботов
	vector BotSpawnPoint = "2491.912842 193.367706 5081.337891";				// задаем точку спавна бота	
	...
	protected float Zone_Radius = 500;											// Радиус триггера на игрока для появления ботов
	...
	void createBotUnit()
	{
		...
		if (m_BotSolder.IsAlive() && m_BotSolder && onRespawnBot)
			GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(respawnBotUnitZel, m_respawnTime * 60000, true, m_BotSolder);
	}
	// ----------------------------- Конец функции спавна бота ------------------------------------- //
	
	// ----------------------------- Функция респавна ботов ----------------------------------------//
	private void respawnBotUnitZel(SurvivorBotBase m_Bot)
	{
	...
	{  
		...
		{	
			for ( int u = 0; u < players.Count(); u++ )
			{
				...
				{
					spawnBotGroup();
					GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).Remove(TriggerPlayersZel);			
				} 
			}
		}
	} 
	...
	void StartMissionAI()
	{
		...
		if (canUseTrigger)
		{
			GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).CallLater(TriggerPlayersZel, 5000, true);			
		}
		else
		{
			spawnBotGroup();
